...
1. 数据调度中心设计
   1. 适配层
   2. 调度分发
   3. 分类管理数据（设计太多种类，用层级管理）
2. 算法层
   1. pipline设计   各个
   2. 暂时不考虑fusion 但预留各个算子的
...
1. 数据采集思路：调试点云文件和检测分析文件 ，
   1. 中间的调试文件最好把数据保存在一个结构体（各个阶段点云，检测状态，），由于算法的pipline导致有些return，后续的点云数据为空，保存点云在最后一步进行；
   2. 检测分析文件，把每次执行的过程写入txt文件
2. 文件命名方式：前缀=时间（精确到秒）+检测状态（成功或者失败）  
3. 保存数据的接口就在这个结构体里面实现
