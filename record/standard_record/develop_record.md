...
<img src="develop_record.assets/image-20250821102944744.png" alt="image-20250821102944744" style="zoom:15%;" />
...
![20250816_192109](develop_record.assets/20250816_192109.gif)
...
解决1：oba_laser_range_min改为10  min_continous_oba_back_scan_size改为0
...
- 货架滤波和TOF2,3,8,9解耦开
- 半径滤波滤掉问题  oba_laser_range_min改为10  min_continous_oba_back_scan_size改为0
- 点云文件时间戳命名
...
| Tof                                       | 说明                               | 使用                 |
| ----------------------------------------- | ---------------------------------- | -------------------- |
| enable_record_obstacle_lidar_back（复用） | 开启TOF2,3,8,9载货下货架不过滤功能 | （300E开启 600关掉） |
| oba_laser_range_min（复用）               | 半径滤波距离参数                   | 10                   |
| min_continous_oba_back_scan_size（复用）  | 半径滤波数量参数                   | 0                    |

| PalletRecognizer           | 说明                                   | 使用             |
| -------------------------- | -------------------------------------- | ---------------- |
| enable_pallet_recognition  | 启用栈板识别功能                       | 自由导航时候开启 |
| pallet_detection_frequency | 栈板检测频率                           | 5                |
| enable_pallet_debug_mode   | 启用栈板识别调试模式(单帧保存 格式ply) | false            |
| enable_pallet_console_log  | 是否启用栈板识别后台日志模式           | false            |
