...
<h2 class="plain-section-title">适用范围</h2>
...
<h2 class="plain-section-title">使用流程</h2>
...
<h2 class="plain-section-title">参数检验清单</h2>
...
<h2 class="plain-section-title">现场交付建议</h2>
...
<h2 class="plain-section-title">相关案例</h2>

### 案例 1：车体参数配置错误（叉车避障模型过大）

**现象**：避障信息中的车体模型尺寸明显大于真实叉车，前方和侧方避障区域被放大。

![叉车避障模型过大](../assets/images/obstacle-2.0/cases/forklift-model-too-large.png)

**原因**：非对称避障模型配置错误。正常导入参数包不会出现此问题；忘记导入参数包时，旧版与当前版本的非对称定义差异可能导致模型异常。

**处理**：MP15 只配置左侧超出车体部分的增量，其他值设为 0；其他车型保持 0，并重新确认参数包和车体模型。

### 案例 2：潜伏车扫到墙体避障

**现象**：机器人靠近墙体时，墙面点云或避障区域被纳入检测，导致不必要的避障或无法通过。
...
**排查**：先确认墙体点云是否稳定进入避障输入，再对照车体包络、侧向避障区域和传感器视场，检查区域是否过宽或模型是否偏移；同时排除地图障碍物重复叠加。

**处理**：根据复现结果调整区域边界、侧向增量和传感器过滤参数，再通过录包回放或现场测试确认机器人可以沿墙安全通过。
