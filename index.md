---
layout: default
title: Glider Record
---

# Glider Record

> **记录之美...**

## 内容概览

### 📝 工作记录
- [标准记录](record/standard_record/std_record) - 日常工作记录和项目总结

### 📚 知识库
- [书籍](repository/book) - 个人书单和阅读记录
- [读书笔记](repository/book_note) - 深度阅读思考
- [GitHub仓库](repository/github) - 代码库和项目
- [专业知识点](repository/professional_knowledge) - 技术知识整理
- [工具包](repository/toolkit) - 开发工具和资源
- [Isaac Sim](repository/isaac-sim-) - 仿真环境配置
- [简历](repository/resume) - 个人简历

### 🎯 生活管理
- [生活管理](life_management/) - 思维模式、理财规划和目标管理

## 特色功能

- ✨ 日常工作记录
- 📖 个人知识库整理
- 💻 开发常用代码库
- 📊 项目记录和总结
- 🎨 丰富的静态资源支持

## 使用说明

本网站基于 Jekyll 构建，支持：
- Markdown 文档渲染
- 图片、音频、视频等多媒体内容
- PDF 文档预览
- 代码高亮显示
- 响应式设计

---

**开始探索** → 点击上方链接浏览不同分类的内容

## 完整内容

完整的文档内容已备份到 `temp_backup/` 目录中，可以根据需要逐步添加。